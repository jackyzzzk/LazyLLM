...
class RerankerDataFormatter(reranker):
    ...
    def forward_batch_input(
        ...
        LOG.info(f"Formatting {len(data)} samples with format: {self.output_format}")

        # Build parallel processing pipeline
        with pipeline() as ppl:
            # Stage 1: Validate data (CPU-bound)
            ppl.validate = RerankerValidateData(
                input_query_key=input_query_key,
                input_pos_key=input_pos_key,
                input_neg_key=input_neg_key
            )
            
            # Stage 2: Format data based on output format (CPU-bound, may expand rows)
            if self.output_format == "flagreranker":
                ppl.format = RerankerFormatFlagReranker(
                    train_group_size=self.train_group_size
                )
            elif self.output_format == "cross_encoder":
                ppl.format = RerankerFormatCrossEncoder()
            elif self.output_format == "pairwise":
                ppl.format = RerankerFormatPairwise()
            else:
                raise ValueError(f"Unknown output format: {self.output_format}")
            
            # Stage 3: Save to file (I/O-bound)
            if self.output_file:
                ppl.save = RerankerSaveToFile(output_file=self.output_file)

        # Execute pipeline
        results = ppl(data)

        LOG.info(f"Formatted {len(results)} samples")
        return results
