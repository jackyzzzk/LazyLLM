...
class RerankerQueryGenerator(reranker):
    ...
    def forward_batch_input(
        ...
        LOG.info(f"Generating queries for {len(data)} passages using parallel pipeline...")

        # Build parallel processing pipeline
        with pipeline() as ppl:
            ppl.build_prompt = RerankerBuildQueryPrompt(
                num_queries=self.num_queries,
                lang=self.lang,
                difficulty_levels=self.difficulty_levels
            )
            ppl.generate = RerankerGenerateQueries(
                llm_serving=self.llm_serving,
                lang=self.lang
            )
            ppl.parse = RerankerParseQueries(
                input_key=input_key,
        ...
        LOG.info(f"Generated {len(results)} query-passage pairs from {len(data)} passages.")
        return results
