...
class EmbeddingQueryGenerator(embedding):
    ...
    def forward_batch_input(
        ...
        **kwargs
        ...
        LOG.info(f"Generating queries for {len(inputs)} passages using parallel pipeline...")

        # Build parallel processing pipeline
        with pipeline() as ppl:
            ppl.build_prompt = EmbeddingBuildQueryPrompt(
                num_queries=self.num_queries,
                lang=self.lang,
                query_types=self.query_types
            )
            ppl.generate = EmbeddingGenerateQueries(
                llm=self.llm,
                lang=self.lang
            )
            ppl.parse = EmbeddingParseQueries(
                input_key=input_key,
        ...
        LOG.info(f"Generated {len(results)} query-passage pairs from {len(inputs)} passages.")
        return results
