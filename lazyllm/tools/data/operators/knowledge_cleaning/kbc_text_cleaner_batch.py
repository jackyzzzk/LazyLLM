...
class KBCTextCleanerBatch(kbc):
    ...
    def forward_batch_input(
        ...
        with pipeline() as ppl:
            ...
            # Stage 2: Build prompts (CPU-bound)
            ppl.build_prompt = KBCBuildCleanPrompt(lang=self.lang)
            
            # Stage 3: Generate cleaned text using LLM (I/O-bound)
            ppl.generate = KBCGenerateCleanedText(llm=self.llm, lang=self.lang)
            
            # Stage 4: Extract cleaned content (CPU-bound)
            ppl.extract = KBCExtractCleanedContent()
            
            # Stage 5: Save cleaned chunks (I/O-bound)
            ppl.save = KBCSaveCleanedChunks(output_key=output_key)
