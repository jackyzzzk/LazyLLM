...
class KBCMultiHopQAGeneratorBatch(kbc):
    ...
    def forward_batch_input(
        ...
        LOG.info(f"Starting multi-hop QA generation for {len(data)} items...")

        # Build parallel processing pipeline
        with pipeline() as ppl:
            # Stage 1: Load chunk files (I/O-bound)
            ppl.load = KBCLoadChunkFile(input_key=input_key)
            
            # Stage 2: Preprocess texts (CPU-bound)
            ppl.preprocess = KBCPreprocessText()
            
            # Stage 3: Extract info pairs (CPU-bound)
            ppl.extract = KBCExtractInfoPairs(lang=self.lang)
            
            # Stage 4: Build prompts (CPU-bound)
            ppl.build_prompt = KBCBuildMultiHopPrompt(lang=self.lang)
            
            # Stage 5: Generate QA using LLM (I/O-bound)
            ppl.generate = KBCGenerateMultiHopQA(llm=self.llm, lang=self.lang)
            
            # Stage 6: Parse QA pairs (CPU-bound)
            ppl.parse = KBCParseQAPairs()
            
            # Stage 7: Save enhanced chunks (I/O-bound)
            ppl.save = KBCSaveEnhancedChunks(output_key=output_key)

        # Execute pipeline
        results = ppl(data)

        LOG.info(f"Multi-hop QA generation completed for {len(results)} items.")
        return results
